...
app.get('/', async (req, res) => {
	...
	try 
		...
		catch (error) { console.error(error.response?.data || error.message); res.status(500).send('Unable to retrieve 				records.'); }
});
...
app.post('/update-cobj', async (req, res) => { 
	...
	try { await axios.post(url, newBook, { headers }); res.redirect('/'); } catch (error) { console.error(error.response?.data || error.message); res.status(500).send('Unable to create record.'); } 
	});
